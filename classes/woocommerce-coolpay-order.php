...
class WC_CoolPay_Order extends WC_Order {
	...
	public function get_transaction_params() {
		$is_subscription = $this->contains_subscription() || $this->is_request_to_change_payment() || WC_CoolPay_Subscription::is_subscription( $this->get_id() );
	...
	public function get_custom_variables() {
		$custom_vars_settings = (array) WC_CP()->s( 'coolpay_custom_variables' );
		$custom_vars          = array();
	...
	public function get_autocapture_setting() {
		...
		$autocapture_default = WC_CP()->s( 'coolpay_autocapture' );
		$autocapture_virtual = WC_CP()->s( 'coolpay_autocapture_virtual' );
